...
func getFieldValue(fieldEntityGroup Field_Entity_Group, entityId int, fieldId int) (value float64, err error) {
	...
	case FE_VHCU:
		return getVhcuFieldValue(entityId, fieldId)
	default:
...
func getVhcuFieldValue(vhcuIndex int, fieldId int) (value float64, err error) {
	vHcuInfo, err := dmiGetVDeviceInfo(vhcuIndex)
	if err != nil {
		return 0, err
	}
	switch fieldId {
	case VHCU_SCLK:
		return HCUClk(vHcuInfo.DvInd)
	case VHCU_TEMP:
		return Temperature(vHcuInfo.DvInd)
	case VHCU_UTILIZATION_RATE:
		return float64(vHcuInfo.VPercent), nil
	case VHCU_USED_MEMORY_BYTES:
		return float64(vHcuInfo.VMemoryUsed), nil
	default:
		err = fmt.Errorf("unknown field %v", fieldId)
		return 0, err
	}
