...
func formatDcuList(groupId string) (hcuList []int, headerStr string, err error) {
	if groupId != "" {
...
func handleGet() {
	hcuList, headerStr, err := formatDcuList(groupId)
	if err != nil {
...
func handleClear() {
	hcuList, _, err := formatDcuList(groupId)
	if err != nil {
		fmt.Printf("Error getting hcu list: %v\n", err)
	}
