...
func queryDcuInfo(hcuIndex int, infoFlags string) error {
	fmt.Printf("+--------------------------+-------------------------------------------------+\n"+
...
func queryNonVerboseGroupInfo(infoFlags string, hcuInGroup []int) error {
	...
	fmt.Println("+==========================+=================================================+")
	if strings.Contains(infoFlags, "a") {
		tmpNameStr, tmpPciId, tmpDcuUniqueId, tmpDcuSerialNumber, tmpVbiosVersion := getIdentifiers(hcuInGroup[0])
		...
		for i := 1; i < len(hcuInGroup); i++ {
			...
			if tmpDcuUniqueId != NOT_APPLICABLE && tmpDcuUniqueId != hcuUniqueId {
				tmpDcuUniqueId = NOT_APPLICABLE
			}
			if tmpDcuSerialNumber != NOT_APPLICABLE && tmpDcuSerialNumber != hcuSerialNumber {
...
func handleInfoFlagsOperations() {
	...
	if groupId != "" {
		...
		err = queryDcuInfo(hcuIndex, infoFlags)
		if err != nil {
